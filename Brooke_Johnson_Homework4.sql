...
SELECT
    m.original_title,
    r.weighted_average_vote AS avg_vote,
    r.votes_1
FROM ratings r
LEFT JOIN movies m ON r."imdb_title_id" = m."imdb_title_id"
ORDER BY 3 DESC
LIMIT 5;
/*
 Question 2
...
SELECT
    a.name,
    SUM(tv.top1000_voters_votes) AS total_votes,
    ROUND(AVG(tv.top1000_voters_rating), 2) AS avg_rating
FROM actors a
 JOIN (
    SELECT
        r.top1000_voters_votes,
        r.top1000_voters_rating,
        r.imdb_title_id,
        tp.imdb_name_id,
        tp.category
    FROM title_principals tp
    LEFT JOIN ratings r ON r."imdb_title_id" = tp."imdb_title_id"
    LEFT JOIN movies m ON m."imdb_title_id" = tp."imdb_title_id"
    WHERE tp.category = 'actress'
        AND m.genre ILIKE '%drama%') tv
ON a."imdb_name_id" = tv."imdb_name_id"
GROUP BY a.name
HAVING SUM(tv.top1000_voters_votes) >= 10000
ORDER BY 3 DESC
LIMIT 5;
...
SELECT
    ...
    ROUND((CAST((COUNT(CASE WHEN vl.avg_vote >= 8.0 THEN vl.imdb_title_id END)) AS NUMERIC)*100)/CAST((COUNT(vl.imdb_title_id)) AS NUMERIC), 1) AS pct_8plus_movies
FROM actors a
LEFT JOIN (
    SELECT
        m.avg_vote,
